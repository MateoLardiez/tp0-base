...
FALTANTE: Debo cerrar la conexion del cliente en el lado del server, enviandole al cliente un NO que quiere decir que no estan los winners todavia. El cliente va a volver a mandar mensaje END, esperando los winners. Solo se envian los winners cuando todos los clientes envien END. Entonces lo que hay que hacer es actualizar el client_socket cada vez que llega un nuevo mensaje END. Para esto el cliente debe mandar tambien el nro de agency asi se sabe cual client_socket actualizar.
Para esto deberia actualizar la forma de recepcion del primer mensaje. Que se envie AGENCY_IDSTART para hacer todo el flujo, sino AGENCY_IDEND, para solo enviar los winners